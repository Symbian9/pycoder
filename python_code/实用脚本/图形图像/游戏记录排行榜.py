...
def rank():
  ...
  for i in range(len(tdb)-1):
    ...
    for j in range(len(tdb)-i-1):#我倒。。讲到这感觉有点难了，不适合初学者了#尽量说明白吧,给个tdb的例子如{'xiaowang':'100','xiaozhang':'200'}#这里很明显第一项是关键字也就是玩家姓名，第二项是分数但是是字符串#tdb[0][1]=?明显是'100'#int()转换成数字再排序
    #下面是冒泡内容，不细讲
